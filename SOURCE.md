...
이 문서는 `sentiment-trading-strategy` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: 감성 트레이딩 전략 (Sentiment Trading Strategy)
- 연구 카테고리: Market Signals & Event Analytics
- 핵심 질문: 단순 감성 신호가 비용을 차감한 뒤에도 방향성 전략으로 의미 있는가?
